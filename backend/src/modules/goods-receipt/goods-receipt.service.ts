...
export class GoodsReceiptService {
  ...
  async cancel(id: number, userId: number, reason?: string) {
    ...
    if (gr.status === 'POSTED') {
      throw new BadRequestException('Posted GR cannot be cancelled. Use reverse instead.');
    }
   ...
   * - Creates a new GR with negative quantities
   * - Deducts stock from FIFO layers (once per item!)
   * - Updates original GR status to REVERSED
   * - Reverts PO and Quotation item quantities
   */
  async reverse(id: number, userId: number, reason?: string) {
    ...
    if (gr.reversedToId) {
      throw new BadRequestException('This GR has already been reversed');
    }
    ...
    try {
      // 1. Create Reverse GR (negative quantities)
      const { docBaseNo, docFullNo } = await this.docNumberingService.generateDocNumber('GR', queryRunner);
      ...
      // 2. Create reverse items and deduct from FIFO (ONCE per item!)
      for (const item of gr.items) {
        // Create reverse item with negative qty
        ...
        // 3. Deduct stock from FIFO - ONLY ONCE per item!
        if (item.productId) {
        ...
        // 5. Revert Quotation Item quantities
        if (item.quotationItemId) {
          await queryRunner.manager.query(`
            UPDATE quotation_items 
            SET qty_received = GREATEST(0, qty_received - $1),
      ...
      // 6. Update PO receive status
      if (gr.purchaseOrderId) {
        await this.poService.updateReceiveStatus(gr.purchaseOrderId);
      }

      // 7. Update original GR status to REVERSED
      gr.status = 'REVERSED';
