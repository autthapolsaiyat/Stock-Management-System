...
const PaymentVoucherPrintPreview: React.FC<PaymentVoucherPrintPreviewProps> = ({
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('pv-print-content');
    if (printContent) {
      ...
      if (printWindow) {
        printWindow.document.write(`
          <html>
            <head>
              <title>ใบสำคัญจ่าย ${voucher?.docNo || ''}</title>
              <style>
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      width={900}
      title="พิมพ์ใบสำคัญจ่าย (Payment Voucher)"
      footer={[
    ...
    >
      {loading ? (
        ...
        <div id="pv-print-content" style={{ padding: 20, background: '#fff', color: '#000', fontFamily: 'Sarabun, sans-serif' }}>
          {/* Header */}
          ...
          <div style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', margin: '15px 0', borderTop: '2px solid #000', borderBottom: '2px solid #000', padding: '8px 0', color: '#000' }}>
            ใบสำคัญจ่าย
            <div style={{ fontSize: 14, color: '#000' }}>PAYMENT VOUCHER</div>
          </div>
            ...
            <tbody>
              <tr>
                <td style={{ width: '60%', border: '1px solid #000', padding: 8 }}>
                  <div style={{ color: '#000' }}><strong>จ่ายให้ / Pay To:</strong></div>
                  <div style={{ fontSize: 16, fontWeight: 'bold', marginTop: 5, color: '#000' }}>{voucher?.supplierName || '-'}</div>
                  <div style={{ marginTop: 5, color: '#000' }}>รหัสผู้จำหน่าย: {voucher?.supplierCode || '-'}</div>
                  {voucher?.supplierTaxId && (
                    <div style={{ marginTop: 5, color: '#000' }}>เลขประจำตัวผู้เสียภาษี: {voucher?.supplierTaxId}</div>
                  )}
                </td>
                <td style={{ width: '40%', border: '1px solid #000', padding: 8 }}>
                  <div style={{ color: '#000' }}><strong>เลขที่ / No.:</strong> {voucher?.docNo || '-'}</div>
                  <div style={{ color: '#000' }}><strong>วันที่ / Date:</strong> {formatDate(voucher?.docDate)}</div>
                  <div style={{ color: '#000' }}><strong>สถานะ:</strong> <span style={{ color: getStatusColor(voucher?.status) }}>{getStatusName(voucher?.status)}</span></div>
                  <div style={{ color: '#000' }}><strong>วิธีจ่าย:</strong> {getPaymentMethodName(voucher?.paymentMethod)}</div>
                  {voucher?.paymentMethod === 'CHEQUE' && (
                    <>
                      <div style={{ color: '#000' }}><strong>เช็คเลขที่:</strong> {voucher?.chequeNo || '-'}</div>
                      <div style={{ color: '#000' }}><strong>ลงวันที่:</strong> {formatDate(voucher?.chequeDate)}</div>
                      <div style={{ color: '#000' }}><strong>ธนาคาร:</strong> {voucher?.chequeBank || '-'}</div>
                    </>
                  )}
                </td>
              </tr>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 15 }}>
            <thead>
              <tr style={{ background: '#f0f0f0' }}>
                ...
                <th style={{ border: '1px solid #000', padding: 6, width: 120, color: '#000' }}>เลขที่อ้างอิง</th>
                ...
                <th style={{ border: '1px solid #000', padding: 6, width: 100, color: '#000' }}>จ่ายชำระ</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item: any, idx: number) => (
                <tr key={idx}>
                  ...
                  <td style={{ border: '1px solid #000', padding: 6, color: '#000' }}>{item.referenceDocNo || '-'}</td>
                  <td style={{ border: '1px solid #000', padding: 6, color: '#000' }}>
                    ชำระค่าสินค้า/บริการ ({item.referenceType || 'GR'})
                  </td>
              ...
              <tr>
                <td style={{ border: '1px solid #000', width: '55%', padding: 10, verticalAlign: 'top' }} rowSpan={4}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 'bold', color: '#000' }}>
                    ({numberToThaiText(Number(voucher?.netAmount || 0))})
                  </div>
                  <div style={{ marginTop: 15, color: '#000' }}><strong>หมายเหตุ:</strong> {voucher?.remark || '-'}</div>
                  {voucher?.bankAccountName && (
                    <div style={{ marginTop: 10, color: '#000' }}><strong>จ่ายจากบัญชี:</strong> {voucher?.bankAccountName}</div>
                  )}
                </td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>รวมยอดเต็ม</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', width: 120, color: '#000' }}>{formatNumber(voucher?.totalAmount || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>ส่วนลดรวม</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>{formatNumber(voucher?.discountAmount || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>หัก ณ ที่จ่ายรวม</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>{formatNumber(voucher?.withholdingTax || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', fontWeight: 'bold', fontSize: 16, color: '#000' }}>จ่ายชำระสุทธิ</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', fontWeight: 'bold', fontSize: 16, color: '#000' }}>{formatNumber(voucher?.netAmount || 0)}</td>
              </tr>
            </tbody>
          </table>
            ...
            <tbody>
              <tr>
                ...
                <td style={{ width: '34%', border: '1px solid #000', padding: 15, textAlign: 'center', color: '#000' }}>
                  <div style={{ marginBottom: 40 }}>ผู้รับเงิน / Received By</div>
                  <div>(..........................................)</div>
                  <div>วันที่/Date ......./......./.......</div>
                </td>
              </tr>
