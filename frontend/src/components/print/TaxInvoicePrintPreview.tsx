  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div id="inv-print-content" style={{ padding: 20, background: '#fff', color: '#000', fontFamily: 'Sarabun, sans-serif' }}>
          ...
          <div style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', margin: '15px 0', borderTop: '2px solid #000', borderBottom: '2px solid #000', padding: '8px 0', color: '#000' }}>
            ...
            <div style={{ fontSize: 14, color: '#000' }}>TAX INVOICE / INVOICE</div>
          </div>
            ...
            <tbody>
              <tr>
                <td style={{ width: '60%', verticalAlign: 'top', border: '1px solid #000', padding: 8 }}>
                  <div style={{ color: '#000' }}><strong>ลูกค้า / Customer:</strong></div>
                  ...
                  <div style={{ marginTop: 10, fontWeight: 'bold', color: '#000' }}>
                    เลขประจำตัวผู้เสียภาษี: {invoice?.customerTaxId || '-'}
                  </div>
                </td>
                <td style={{ width: '40%', verticalAlign: 'top', border: '1px solid #000', padding: 8 }}>
                  <div style={{ color: '#000' }}><strong>เลขที่/No.:</strong> {invoice?.docFullNo || '-'}</div>
                  <div style={{ color: '#000' }}><strong>วันที่/Date:</strong> {formatDate(invoice?.docDate)}</div>
                  <div style={{ color: '#000' }}><strong>อ้างอิง QT:</strong> {invoice?.quotationDocNo || '-'}</div>
                  <div style={{ color: '#000' }}><strong>เงื่อนไขชำระ:</strong> {invoice?.paymentTermDays || 30} วัน</div>
                  <div style={{ color: '#000' }}><strong>กำหนดชำระ:</strong> {formatDate(invoice?.dueDate)}</div>
                </td>
              </tr>
            </tbody>
          </table>
            ...
            <tbody>
              <tr>
                <td style={{ border: '1px solid #000', width: '60%', padding: 8, verticalAlign: 'top' }} rowSpan={4}>
                  <div style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 10, color: '#000' }}>
                    ({numberToThaiText(Number(invoice?.grandTotal || 0))})
                  </div>
                  <div style={{ color: '#000' }}><strong>หมายเหตุ:</strong> {invoice?.remark || '-'}</div>
                </td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>รวมเป็นเงิน / Subtotal</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', width: 120, color: '#000' }}>{formatNumber(invoice?.subtotal || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>ส่วนลด / Discount</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>{formatNumber(invoice?.discountTotal || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>ภาษีมูลค่าเพิ่ม 7% / VAT 7%</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', color: '#000' }}>{formatNumber(invoice?.taxAmount || 0)}</td>
              </tr>
              <tr>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', fontWeight: 'bold', fontSize: 16, color: '#000' }}>รวมทั้งสิ้น / Grand Total</td>
                <td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', fontWeight: 'bold', fontSize: 16, color: '#000' }}>{formatNumber(invoice?.grandTotal || 0)}</td>
              </tr>
            </tbody>
          </table>
